...
- A pulsing Horizon-Rose dot lights up the contact row as it appears on
  the right; the "Jump to" link scrolls and highlights it for 1.2s.
...
- **Considered**: every line of typography earns its place; nothing
  decorative; the wordmark is sacred; ember-rose is reserved.
...
4. **Every AI action is reviewable and revertable.** Tool calls render
   as structured cards in the chat with: what changed, in human
   language; a "Jump to" link that scrolls + highlights the right-pane
   row; and, for destructive calls, an Undo affordance.
